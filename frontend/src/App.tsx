...
export function App() {
  ...
  const screen = route === "/app/dashboard" ? <DashboardScreen data={data} tutor={pref.tutor} /> : route === "/app/daily-lesson" ? <DailyLessonScreen /> : route === "/app/conversation" ? <ConversationScreen key={`${account.learner_id}:${pref.tutor.tutor_id}`} account={account} tutor={pref.tutor} languageMode={pref.language_mode} /> : route === "/app/progress" ? <ProgressDetailScreen /> : route === "/app/feedback" ? <FeedbackScreen /> : route === "/app/subscription" ? <SubscriptionScreen /> : route === "/app/founder" ? <FounderScreen /> : <SettingsScreen data={data} tutor={pref.tutor} onChange={() => navigate("/onboarding")} />;
