...
import { ConversationScreen, DashboardScreen } from "../screens/ExperienceScreens";
import { TutorPicker } from "../screens/TutorPicker";
import { account, ananya, arjun, dashboard, preference } from "./fixtures";
...
beforeEach(() => {
  ...
  Object.defineProperty(URL, "createObjectURL", {
    configurable: true,
    value: vi.fn().mockReturnValue("blob:browser-acceptance"),
  });
  ...
  vi.spyOn(api, "turn").mockResolvedValue({
    turn_id: "browser-acceptance-turn",
    tutor_message: "Thanks for sharing.",
    next_question: "What happened next?",
    incorrect_span: "go",
    corrected_form: "went",
    correction_explanation: "Use the past tense here.",
    vocabulary_suggestions: ["confident", "routine"],
    telugu_explanation: "ఇక్కడ భూతకాలం ఉపయోగించండి.",
  });
...
describe("deterministic browser acceptance", () => {
  for (const [name, width] of [["mobile", 390], ["tablet", 768], ["desktop", 1440]] as const) {
    it(`renders ${name} viewport`, () => {
      Object.defineProperty(window, "innerWidth", { configurable: true, value: width });
      const { unmount } = render(<RouterProvider><DashboardScreen data={dashboard} tutor={ananya} /></RouterProvider>);
      expect(screen.getByText("Ready for today’s conversation?")).toBeVisible();
      unmount();
    });
  }

  it("selects female and male tutors accessibly", async () => {
    render(<RouterProvider><TutorPicker /></RouterProvider>);
    const female = await screen.findByRole("radio", { name: /Ananya/ });
    const male = screen.getByRole("radio", { name: /Arjun/ });
    ...
    expect(api.savePreference).toHaveBeenCalledWith("arjun", "ENGLISH");
  });

  it("renders focused voice feedback and fuller text-mode coaching without diagnostics", async () => {
    render(<RouterProvider><ConversationScreen account={account} tutor={ananya} telugu /></RouterProvider>);
    ...
    expect(await screen.findByLabelText("Current tutor response")).toHaveTextContent("Thanks for sharing");
    expect(screen.getByLabelText("Current grammar correction")).toHaveTextContent("went");
    expect(screen.getByText(/భూతకాలం/)).toBeVisible();
    expect(screen.queryByText(/Provider: openai/)).not.toBeInTheDocument();
    const avatar = screen.getByLabelText(/status: thinking/);
  ...
  it("provides text fallback when microphone is unavailable", async () => {
  ...
  it("produces no critical console errors", () => {
    const spy = vi.spyOn(console, "error").mockImplementation(() => undefined);
    render(<RouterProvider><DashboardScreen data={dashboard} tutor={arjun} /></RouterProvider>);
