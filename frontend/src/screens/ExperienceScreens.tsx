...
export function ConversationScreen({account,tutor,telugu}:{account:Account;tutor:Tutor;telugu:boolean}){
 ...
 const submit=useCallback(async(text:string,retryKey?:string)=>{
  ...
  try{
   dispatch("THINKING");
   const result=await api.turn(id,learnerText,telugu,key);
 ...
 },[id,loadSpeech,telugu,tutor.display_name]);
