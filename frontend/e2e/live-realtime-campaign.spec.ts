...
const corpus:Array<{category:Category;text:string}>=[
  ...
  {category:"mixed",text:"దీని ధర ఎంత అని English లో ఎలా చెప్పాలి?"},{category:"mixed",text:"నేను Hyderabad వెళ్లాలి అని English లో ఎలా చెప్పాలి?"},
  {category:"mixed",text:"Market lo bargaining ఎలా start చేయాలి?"},{category:"mixed",text:"Today meeting లో project update ఎలా చెప్పాలి?"},
  {category:"mixed",text:"Tomorrow leave కావాలి, polite English sentence చెప్పండి."},
...
test("30 spoken turns meet Realtime latency and language gates",async({page,request})=>{
  ...
  for(const [localIndex,turn] of activeCorpus.entries()){
    ...
    const sendEnd=await page.evaluate(async audio=>{
      ...
      for(let offset=0;offset<bytes.length;offset+=4800){const chunk=bytes.subarray(offset,offset+4800);let binary="";for(const value of chunk)binary+=String.fromCharCode(value);dc.send(JSON.stringify({type:"input_audio_buffer.append",audio:btoa(binary)}));await new Promise(resolve=>setTimeout(resolve,100))}
      const speechEnd=performance.now();const silence=new Uint8Array(48_000);for(let offset=0;offset<silence.length;offset+=4800){const chunk=silence.subarray(offset,offset+4800);let binary="";for(const value of chunk)binary+=String.fromCharCode(value);dc.send(JSON.stringify({type:"input_audio_buffer.append",audio:btoa(binary)}));await new Promise(resolve=>setTimeout(resolve,100))}return speechEnd;
    },pcm);
    ...
    const observed=await page.evaluate(({from,end,responseId})=>{
      ...
      const responseIds=new Set(events.filter(item=>item.at>=end&&item.event.type==="response.output_audio_transcript.done").map(item=>String(item.event.response_id??"")));
    ...
    if(turn.category==="telugu"||turn.category==="mixed"){const answer=String(observed.transcript??"");expect(answer).toMatch(/[A-Za-z]/);expect(answer).toMatch(/[\u0c00-\u0c7f]/);expect(answer).not.toMatch(/[\u0c80-\u0cff\u0400-\u04ff\u0600-\u06ff\u4e00-\u9fff\ufffd]/)}
  }
  ...
  if(activeCorpus.some(item=>item.category==="mixed")){const mixedChecks:Array<[string,RegExp]>=[["దీని ధర ఎంత అని English లో ఎలా చెప్పాలి?",/how much|price/i],["నేను Hyderabad వెళ్లాలి అని English లో ఎలా చెప్పాలి?",/Hyderabad/i],["Market lo bargaining ఎలా start చేయాలి?",/bargain/i],["Today meeting లో project update ఎలా చెప్పాలి?",/project update/i],["Tomorrow leave కావాలి, polite English sentence చెప్పండి.",/leave/i]];for(const [input,expected] of mixedChecks){const answer=await sendExactText(input);exactQuality.push(answer);expect(answer).toMatch(/[\u0c00-\u0c7f]/);expect(answer).toMatch(expected)}}
