...
The initial Alembic revision is `0001_learner_onboarding`; the current RC schema head is `0018_opening_turns`. SQLite supports local development and isolated tests; model types and constraints remain PostgreSQL-compatible. Foreign keys express ownership and turn uniqueness protects transcript order. Subscription uniqueness protects one commercial lifecycle per learner, while conversation-scoped STT idempotency protects one privacy-minimised transcription identity per key. Dated provider-call events preserve per-dispatch outcome, duration and usage attribution without storing raw audio or provider payloads. Opening tutor attempts are explicitly classified so the server-owned greeting is idempotent and excluded from learner-turn context and quota accounting.

Later revisions add lessons, richer feedback provenance, daily activity/streaks, progress snapshots, subscriptions, entitlements, provider attempts, and usage evidence. Audio remains transient request data or private object-storage metadata rather than a relational database blob.
...
`0005_operations_observability` adds compound audit and refresh-family indexes plus deterministic audio-cleanup scan indexes. Audio assets track retry count, allow-listed failure reason, and deletion confirmation time. Security audit rows are append-only: no learner-facing update or delete API exists.
