...
`UserAccount` stores normalized unique email, bcrypt hash, status, verification flag, timestamps, and last login. Each newly registered `Learner` has one unique `user_account_id`. `RefreshToken` stores only a SHA-256 token hash, issue/expiry/revocation timestamps, replacement linkage, bounded user-agent data, and no raw IP address.
