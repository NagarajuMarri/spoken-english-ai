...
Stop rollout and leave the previous compatible image serving. Compare the reported revision with `0011_password_recovery`. Re-run the idempotent upgrade only after resolving the failed migration. Prefer a forward repair; rehearse any downgrade on a restored copy first.
