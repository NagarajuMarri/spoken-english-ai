...
Stop rollout and leave the previous compatible image serving. Compare the reported revision with `0017_provider_call_events`. Re-run the idempotent upgrade only after resolving the failed migration. Prefer a forward repair; rehearse any downgrade on a restored copy first.
