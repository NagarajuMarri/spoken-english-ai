...
`POST /api/v1/auth/register`, `/login`, `/refresh`, `/logout`, `/logout-all`, and `GET /me` implement the session lifecycle. Password recovery uses `POST /api/v1/auth/password-reset/request`, `/password-reset/validate`, and `/password-reset/confirm`. The request response is neutral for known and unknown emails; validation never places the token in the URL sent to the API; confirmation consumes the token and revokes all existing access and refresh sessions. Access tokens are bearer JWTs with a session epoch; refresh tokens are opaque and rotate on every successful refresh. Reuse, expiry, invalid issuer, invalid audience, revoked sessions, and inactive accounts produce structured authentication errors.
